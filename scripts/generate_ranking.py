...
def fetch_ranking(keyword="", hits=20, service="digital", floor="videoa"):
...
categories: ["Ranking"]
draft: false
description: "{date_str}更新の{genre_name}{type_label}ランキングTOP20。FANZAの売れ筋作品をサンプル画像付きで紹介。"
cover:
...
## {genre_name} {type_label}ランキング TOP20
...
"""
    ...
    for item in items:
        ...
        actress_text = "・".join(item["actresses"][:3]) if item["actresses"] else ""
...
"""
        if actress_text:
            md += f"    <p><strong>出演:</strong> {actress_text}</p>\n"
        if genre_text:
